...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        #       outputs                                                             #
        #############################################################################
        w1 = np.dot(X, self.weights['W1'])
        b1 = w1 + self.weights['b1']
        s1 = self.sigmoid(b1)
        w2 = np.dot(s1, self.weights['W2'])
        b2 = w2 + self.weights['b2']
        sm = self.softmax(b2)
        loss =  self.cross_entropy_loss(sm, y)
        accuracy = self.compute_accuracy(sm, y)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        #############################################################################
        ...
        self.gradients['W2'] = s1.T @ grad
        self.gradients['b2'] = np.ones(m).T @ grad
        grad = self.weights['W2'] @ grad.T
        grad = grad.T * np.array(self.sigmoid_dev(b1))
        self.gradients['W1'] = X.T @ grad
        self.gradients['b1'] = np.ones(m).T @ grad
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
